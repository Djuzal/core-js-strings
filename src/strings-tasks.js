...
function formatTime(minutes, seconds) {
  ...
  const result = minutesStr.concat(secondsStr);
  return result;
}
...
function orderAlphabetically(str) {
  ...
  const result = charArray.sort((a, b) => a.localeCompare(b)).join('');
  return result;
}
...
function countVowels(str) {
  ...
  for (let i = 0; i <= str.length; i += 1) {
    if (vowels.includes(testString[i])) result += 1;
  }
  return result;
}
...
function isPalindrome(str) {
  const cleanStr = str.replace(/[^a-z0-9]/gi, '').toLocaleLowerCase();
...
function invertCase(str) {
  ...
  for (let i = 0; i <= str.length; i += 1) {
    ...
    inversCaseString += char;
  }
  return inversCaseString;
}
...
function extractNameFromTemplate(value) {
  ...
  const firstName = valueSplit[1];
  const lastName = valueSplit[2];
  return `${firstName} ${lastName.replace(/[^a-z]/gi, '')}`;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const char = str[i];
    if (
      upperCase.includes(char) === false &&
      lowerCase.includes(char) === false
    ) {
      result += char;
...
function getCardId(value) {
  ...
  const result = deck.indexOf(value);
  return result;
}
